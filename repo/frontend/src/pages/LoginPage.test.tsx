...
describe("LoginPage", () => {
  ...
  it("shows the server detail message on a 423 (account locked) response", async () => {
    mockLogin.mockRejectedValue({
      response: { status: 423, data: { detail: "Account locked due to too many attempts." } }
    });
    render(<LoginPage onSuccess={() => {}} />);
  ...
  it("falls back to lock message when 423 detail is absent", async () => {
    mockLogin.mockRejectedValue({ response: { status: 423 } });
    render(<LoginPage onSuccess={() => {}} />);
    ...
    await waitFor(() => {
      expect(
        screen.getByText("Account is locked. Please wait before retrying.")
      ).toBeInTheDocument();
    });
  });
});
